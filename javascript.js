$(document).ready(function() {
  $('.story__slider').slick({
    ...
    prevArrow: '<button class="story__prev-button story__disabled" disabled>\
        <svg width="18" height="15" viewBox="0 0 18 15" fill="none" xmlns="http://www.w3.org/2000/svg">\
  ...
  $('.story__slider').on('afterChange', function(){
    $('.story__prev-button, .story__next-button').prop("disabled", false).removeClass("story__disabled")
    if ($('.story__slide:first-child').hasClass("slick-active")) {
      $('.story__prev-button').prop("disabled", true).addClass("story__disabled")
    } else if ($('.story__slide:last-child').hasClass("slick-active")) {
      $('.story__next-button').prop("disabled", true).addClass("story__disabled")
    }
  })
})
